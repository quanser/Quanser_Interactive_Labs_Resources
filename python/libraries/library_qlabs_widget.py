...
class QLabsWidget:
    ...
    def spawn(self, widgetType, location, rotation, scale, colour, measuredMass=0, IDTag=0, properties="", waitForConfirmation=True):
        ...
        c.payload = bytearray(struct.pack(">IfffffffffffffBI", widgetType, location[0], location[1], location[2], rotation[0], rotation[1], rotation[2], scale[0], scale[1], scale[2], colour[0], colour[1], colour[2], measuredMass, IDTag, len(properties)))
