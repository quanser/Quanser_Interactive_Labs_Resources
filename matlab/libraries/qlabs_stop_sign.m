classdef qlabs_stop_sign < handle
    ...
    methods
        ...
        %%
        function status = spawn_id_degrees(obj, qlabs, actor_number, location, rotation, scale, wait_for_confirmation)
            if nargin == 6
                wait_for_confirmation = true;
            end     
            ...
            status = qlabs_common_spawn_id(qlabs, actor_number, obj.ID_STOP_SIGN, location, rotation/180*pi, scale, configuration, wait_for_confirmation);
        end
		%%
        ...
        %%
        function num_destroyed = destroy(obj, qlabs, actor_number)
            
            num_destroyed = qlabs_common_destroy_spawned_actor(qlabs, obj.ID_STOP_SIGN, actor_number);
        end
