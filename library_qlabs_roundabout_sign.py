...
class qlab_roundabout_sign:
    ...
    def spawn(self, qlabs, device_num, location, rotation, scale, wait_for_confirmation=True):
        return qlabs.spawn(device_num, self.ID_ROUNDABOUT_SIGN, location[0], location[1], location[2], rotation[0], rotation[1], rotation[2], scale[0], scale[1], scale[2], 0, wait_for_confirmation)
