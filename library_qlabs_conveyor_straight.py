...
class QLabsConveyorStraight:
    ...
    def spawn(self, qlabs, deviceNumber, location, rotation, sections=0, waitForConfirmation=True):
        return qlabs.spawn(deviceNumber, self.ID_CONVEYOR_STRAIGHT, location[0], location[1], location[2], rotation[0], rotation[1], rotation[2], 1, 1, 1, sections, waitForConfirmation)
 
    def spawn_degrees(self, qlabs, deviceNumber, location, rotation, sections=0, waitForConfirmation=True):
        return qlabs.spawn(deviceNumber, self.ID_CONVEYOR_STRAIGHT, location[0], location[1], location[2], rotation[0]/180*math.pi, rotation[1]/180*math.pi, rotation[2]/180*math.pi, 1, 1, 1, sections, waitForConfirmation)
 
    def set_speed(self, qlabs, deviceNumber, speed):
        c = CommModularContainer()
        c.classID = self.ID_CONVEYOR_STRAIGHT
        c.deviceNumber = deviceNumber
        c.deviceFunction = self.FCN_CONVEYOR_STRAIGHT_SET_SPEED
        ...
        if (qlabs.send_container(c)):
            c = qlabs.wait_for_container(self.ID_CONVEYOR_STRAIGHT, deviceNumber, self.FCN_CONVEYOR_STRAIGHT_SET_SPEED_ACK)
